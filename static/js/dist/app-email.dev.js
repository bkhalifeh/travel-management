...
document.addEventListener('DOMContentLoaded', function () {
  (function () {
    var emailList = document.querySelector('.email-list'),
        ...
        commentManagement = document.querySelectorAll('.option-cm'),
    ...
    emailFilterByFolders.forEach(function (emailFilterByFolder) {
      emailFilterByFolder.addEventListener('click', function (e) {
        ...
        appOverlay.classList.remove('show'); // Remove active class from each folder filters

        Helpers._removeClass('active', emailFilterByFolders); // Add active class to selected folder filters


        currentTarget.classList.add('active');
        emailListItems.forEach(function (emailListItem) {
          // If folder filter is Inbox
          //   if (currentTargetData == 'mycomment') {
          //     emailListItem.classList.add('d-block');
          //     emailListItem.classList.remove('d-none');
          //   } else
          //console.log(currentTargetData);
          if (currentTargetData === 'managecomment') {
            commentManagement.forEach(function (cm) {
          ...
          if (emailListItem.hasAttribute('data-' + currentTargetData)) {
            emailListItem.classList.add('d-block');
            emailListItem.classList.remove('d-none');
          } else {
    ...
    if (emailListDelete) {
      emailListDelete.addEventListener('click', function (e) {
        var selectedComment = [];
        emailListItemInputs.forEach(function (emailListItemInput) {
          if (emailListItemInput.checked && emailListItemInput.hasAttribute('data-managecomment')) {
            // emailListItemInput.parentNode.closest('li.email-list-item').remove();
            selectedComment.push(emailListItemInput.getAttribute('id').substr(1));
          }
        });
        selectAllEmails.indeterminate = false;
    ...
    initSelect2(); // Scroll to bottom on reply click
